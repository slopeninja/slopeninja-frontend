...
const HighwayIcon = ({ highwayNumber = '00', width = 54, height = 54 }) => {
  let fontSize = '28';
  let fontWeight = '400';
  let translate = 'translate(10 8)';

  if (highwayNumber.length > 2) {
    fontSize = '20';
    fontWeight = '700';
    translate = 'translate(9.2 6)';
  }
  ...
  return (
    <svg width={width} height={height} viewBox="0 0 54 54" xmlns="http://www.w3.org/2000/svg">
      <g fill="none" fillRule="evenodd">
        ...
        <text transform={translate} fontSize={fontSize} fontFamily="Lato" fill="#4A4A4A" fontWeight={fontWeight}>
          <tspan x="0" y="28.592">{ highwayNumber }</tspan>
        </text>
      </g>
    </svg>
  );
